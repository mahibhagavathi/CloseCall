...
/* ── File uploader — hide the default label overlap ── */
...
def lead_pill(v):
    fg, bg = LEAD_COLORS.get(v, ("#1e40af","#eef2ff"))
    return pill(v, fg, bg)
...
def score_dots(n):
    ...
    for i in range(1, 6):
        ...
        html += (f'<span style="display:inline-block;width:10px;height:10px;'
...
def label(text):
    return (f'<div style="font-size:0.62rem;font-weight:700;color:var(--muted);'
            f'text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.3rem;">{text}</div>')


# ──────────────────────────────────────────────────────────────
# SIDEBAR LOGO (reused across pages)
# ──────────────────────────────────────────────────────────────
...
def render_home():
    with st.sidebar:
        ...
        <div style="padding:0 1.2rem;font-size:0.77rem;color:#4a4642;line-height:2.1;">
          <div>🎧&nbsp; Reads every support call transcript</div>
          <div>💬&nbsp; Spots objections &amp; key issues</div>
          <div>📊&nbsp; Tracks sentiment &amp; tone</div>
          <div>🔥&nbsp; Scores lead quality automatically</div>
          <div>⭐&nbsp; Rates agent performance</div>
          <div>✉️&nbsp; Writes follow-up emails for you</div>
          <div>📋&nbsp; Suggests clear next actions</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    <div style="padding:3.5rem 0 2.8rem;">
      ...
      <div style="font-size:0.97rem;color:var(--muted);max-width:460px;line-height:1.8;">
        Drop in your customer support transcripts. CloseCall's AI reads them,
        finds what matters, and tells your team exactly what to do next.
      </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col_a:
        ...
        <div style="background:var(--surface);border:1.5px solid var(--border);
          ...
          <div style="font-size:0.82rem;color:var(--muted);line-height:1.75;margin-bottom:1.2rem;">
            Upload a <code style="background:var(--bg);padding:1px 5px;border-radius:3px;">.csv</code> file
            with a <code style="background:var(--bg);padding:1px 5px;border-radius:3px;">transcript</code>
            column. Optional: add <code style="background:var(--bg);padding:1px 5px;border-radius:3px;">id</code>
            and <code style="background:var(--bg);padding:1px 5px;border-radius:3px;">company</code> columns.
          </div>
        </div>
        """, unsafe_allow_html=True)

        # File uploader sits OUTSIDE the card div so its button doesn't overlap
    ...
    with col_b:
        ...
        <div style="background:var(--surface);border:1.5px solid var(--border);
          ...
          <div style="font-size:0.82rem;color:var(--muted);line-height:1.75;margin-bottom:1.2rem;">
            100 real-world support calls across fashion, real estate, finance,
            healthcare &amp; tech. No upload needed.
          </div>
...
# What we show per-step while processing
STEP_DESCRIPTIONS = [
    ("Reading the transcript",      "Pulling out what was actually said on the call."),
    ("Understanding the customer",  "Figuring out their mood, concerns, and intent."),
    ("Spotting issues & objections","Finding what held them back or frustrated them."),
    ("Scoring the lead",            "Deciding if this is a Hot, Warm, or Cold opportunity."),
...
def render_loading():
    ...
    with col:
        ...
        # ── Live "what's happening now" panel ─────────────────
        status_panel = st.empty()

        st.markdown("<div style='height:1.6rem'></div>", unsafe_allow_html=True)

        # ── Step breakdown legend ─────────────────────────────
    ...
    # ── Run analysis — callback updates UI ──────────────────
    def cb(i, total, call_id):
        ...
        # Current step panel cycles through STEP_DESCRIPTIONS based on position
...
def render_dashboard():
    ...
    n     = len(df)
    note  = (f" · Filtered to {n} of {len(df_all)}" if n < len(df_all)
             else (f" · Showing {len(df_all)} of {total_fetched}" if len(df_all) < total_fetched
                   else ""))
    ...
    <div style="padding:1.8rem 0 1.8rem;border-bottom:1.5px solid var(--border);
      ...
      <div style="font-family:'Lora',serif;font-size:1.9rem;color:var(--text);
        ...
        <span style="font-family:'Inter',sans-serif;font-size:0.72rem;font-weight:500;
          color:var(--muted);margin-left:0.9rem;">Dashboard</span>
      </div>
...
def render_kpis(df, total_fetched):
    ...
    for col, accent, lbl, big, sub in [
        (k1, "#1d4ed8", "Calls Analyzed",     str(n),        note),
        ...
        with col:
            st.markdown(f"""
            <div style="background:var(--surface);border:1.5px solid var(--border);
              border-top:3px solid {accent};border-radius:10px;padding:1.3rem 1.5rem;">
              <div style="font-size:0.61rem;font-weight:700;color:var(--muted);
                text-transform:uppercase;letter-spacing:0.11em;margin-bottom:0.5rem;">{lbl}</div>
              <div style="font-size:2.3rem;font-weight:800;color:var(--text);line-height:1;
                letter-spacing:-0.04em;">{big}</div>
              <div style="font-size:0.7rem;color:var(--muted);margin-top:0.4rem;">{sub}</div>
            </div>
            """, unsafe_allow_html=True)
...
def render_charts(df):
    ...
    with c2:
        ...
        fig2.update_layout(showlegend=True, **_chart_layout())
...
def render_table_section(df):
    ...
    for _, r in fdf.head(60).iterrows():
        ...
        <tr style="border-bottom:1px solid #f6f5f2;">
          <td style="padding:11px 14px;font-weight:600;color:var(--text);white-space:nowrap;">{r['id']}</td>
          <td style="padding:11px 14px;color:#4a4642;">{r['company']}</td>
          <td style="padding:11px 14px;">{sent_pill(r.get('sentiment','—'))}</td>
          <td style="padding:11px 14px;">{lead_pill(r.get('lead_type','—'))}</td>
          <td style="padding:11px 14px;color:#4a4642;max-width:230px;line-height:1.45;">{obj}</td>
          <td style="padding:11px 14px;color:#4a4642;max-width:210px;line-height:1.45;">{nxt}</td>
        </tr>"""
      ...
      <table style="width:100%;border-collapse:collapse;font-size:0.81rem;">
        <thead>
          <tr style="background:#faf9f7;border-bottom:1.5px solid var(--border);">
            {''.join(
              f'<th style="padding:9px 14px;text-align:left;font-size:0.61rem;font-weight:700;'
              f'color:var(--muted);text-transform:uppercase;letter-spacing:0.1em;white-space:nowrap;">{h}</th>'
              for h in ["Call ID","Company","Sentiment","Lead","Key Objection","Next Step"]
            )}
          </tr>
        </thead>
        <tbody>{rows}</tbody>
...
def render_detail(df):
    ...
    <div style="font-size:0.71rem;color:var(--muted);margin-bottom:0.75rem;">
      Select a call to see the full transcript, AI analysis, next actions,
    ...
    def chip(lbl, val, fg, bg):
        return f"""<div style="background:{bg};border-radius:8px;padding:0.85rem 1.1rem;">
          <div style="font-size:0.61rem;font-weight:700;color:{fg}90;text-transform:uppercase;
            letter-spacing:0.1em;margin-bottom:0.2rem;">{lbl}</div>
          <div style="font-size:0.87rem;font-weight:700;color:{fg};">{val}</div>
        </div>"""

    sfg, sbg = SENT_COLORS.get(r.get("sentiment","Neutral"), ("#4b5563","#f3f4f6"))
    lfg, lbg = LEAD_COLORS.get(r.get("lead_type","Cold"),    ("#1e40af","#eef2ff"))

    c1,c2,c3,c4 = st.columns(4)
    with c1: st.markdown(chip("Sentiment",    r.get("sentiment","—"),          sfg, sbg), unsafe_allow_html=True)
    with c2: st.markdown(chip("Lead Type",    r.get("lead_type","—"),           lfg, lbg), unsafe_allow_html=True)
    with c3: st.markdown(chip("Resolution",   r.get("resolution_status","—"), "#1c1a18","#faf9f7"), unsafe_allow_html=True)
    with c4: st.markdown(chip("Agent Score",  f"{r.get('rep_score','—')}/5",  "#1c1a18","#faf9f7"), unsafe_allow_html=True)
    ...
    with t1:
        ...
        display = (content
                   .replace("**Sales Rep**", "<strong style='color:#1d4ed8;'>🎙 Agent</strong>")
        ...
        <div style="background:#faf9f7;border:1.5px solid var(--border);border-radius:10px;
          padding:1.3rem 1.7rem;font-size:0.82rem;color:#4a4642;line-height:2;
          max-height:370px;overflow-y:auto;">{display}</div>
        """, unsafe_allow_html=True)
    ...
    with t2:
        ...
        def dp(lbl, val):
            return f"""<div style="background:var(--surface);border:1.5px solid var(--border);
              ...
              <div style="font-size:0.61rem;font-weight:700;color:var(--muted);
                text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.28rem;">{lbl}</div>
        ...
        with ib:
            ...
            <div style="background:var(--surface);border:1.5px solid var(--border);
              border-radius:9px;padding:1rem 1.2rem;margin-bottom:0.65rem;">
              <div style="font-size:0.61rem;font-weight:700;color:var(--muted);
                text-transform:uppercase;letter-spacing:0.1em;margin-bottom:0.3rem;">Agent Score</div>
    ...
    with t4:
        ...
        if st.session_state[ek] is None:
            ...
            <div style="font-size:0.82rem;color:var(--muted);margin-bottom:1.2rem;line-height:1.75;">
              ...
              references the customer's issue, resolution, and agreed next steps.
            </div>
            """, unsafe_allow_html=True)
